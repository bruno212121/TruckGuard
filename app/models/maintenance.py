...
class Maintenance(db.Model):
    ...
    def from_json(maintenance_json):
        ...
        last_maintenance_mileage = maintenance_json.strftime('%Y-%m-%d %H:%M:%S')
        status = maintenance_json.get('status')

        return Maintenance(id=id, 
                           truck_id=truck_id, 
                           maintenance_interval=maintenance_interval, 
                           accumulated_km=accumulated_km, last_maintenance_mileage=last_maintenance_mileage,
                           status=status)
